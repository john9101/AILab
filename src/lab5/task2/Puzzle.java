...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for (int row = 0; row < MAX_ROW; row++) {
			for (int col = 0; col < MAX_COL; col++) {
				int currentStateTile = currentState.getTile(row, col);
				int goalStateTile = goalState.getTile(row, col);
				if (currentStateTile != 0 && currentStateTile != goalStateTile) {
					output++;
				}
			}
		}
	...
	public int computeH2(Node currentState) {
		...
		for (int tile = 1; tile < MAX_ROW * MAX_COL ; tile++) {
			int[] currentStateLocation = currentState.getLocation(tile);
			int[] goalStateLocation = goalState.getLocation(tile);
			result += PuzzleUtils.manhattanDistance(currentStateLocation, goalStateLocation);
		}
